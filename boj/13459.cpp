...
public:
    ...
    // 보드를 cnt번 더 기울일 수 있을 때 가능한 경우를 모두 탐색
    bool backtrack(int cnt) {
        // 기울일 수 있는 횟수가 있고 몇번 더 기울여 성공한다면 true를 반환
        if (cnt) for (int d = 0; d < 4; d++) if (backtrack(cnt, d)) return true;
        // true를 반환하지 못했다면 실패했으므로 false를 반환
        return false;
    }
    // 보드를 cnt번 더 기울일 수 있고
    // 현재 방향이 {위쪽, 아래쪽, 왼쪽, 오른쪽}[dir]일 때
    bool backtrack(int cnt, int dir) {
        ...
        // 빨간 구슬이 구멍에 빠졌다면 true를 반환
        // 그렇지 않다면 보드를 더 기울여 빨간 구슬을 구멍에 넣는다
        return red_in ? true : b.backtrack(cnt - 1);
    }
} brd;
...
int main() {
    ...
    // 빨간 구슬만 구멍에 넣을 수 있는지 여부를 출력
    printf("%d\n", brd.backtrack(MAX_TILT));
