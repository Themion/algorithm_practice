...
void spread_dust()
{
    ...
    for (int i = 0; i < R; i++) for (int j = 0; j < C; j++)
    {
        ...
        // 미세먼지가 있는 좌표의 주변 좌표에 먼지 확산이 가능하다면
        for (auto c : { coord(i, j - 1), coord(i, j + 1),
                        coord(i - 1, j), coord(i + 1, j) })
            if (valid(c))
            {
