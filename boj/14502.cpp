...
int fill_virus()
{
    ...
    while (!q.empty())
    {
        ...
        for (coord crd: { coord(c._y + 1, c._x), coord(c._y, c._x + 1),
            ...
            if (valid(crd) && m[crd._y][crd._x] == 0)
            {
...
// 벽을 3개 세워서 fill_virus를 호출
int make_wall(coord c, int cnt)
{
    // 벽을 3개 세웠다면 공간을 바이러스로 채워 안전한 칸의 수를 계산
    if (cnt == 3) return fill_virus();
...
int main()
{
    ...
    // 연구실의 상태를 입력받으면서 바이러스의 좌표는 따로 저장
    for (int i = 0; i < N; i++) for (int j = 0; j < M; j++)
    {
        scanf("%d", &map[i][j]);
