...
bool fill(int idx)
{
    ...
    for(int i = 1; i <= 9; i++) 
        //i가 해당 칸을 포함하는 행, 열, 블록 안에 없을 때
        if (!row[x][i] && !col[y][i] && !blk[x / 3][y / 3][i])
        {
...
int main()
{
    ...
    for(int i = 0; i < 9; i++) for (int j = 0; j < 9; j++)
    {
        ...
        if (row[i][buf] || col[j][buf] || blk[i / 3][j / 3][buf]) flag = false;
        //그렇지 않다면 현재 칸이 해당된 행, 열, 블록에 val이 등장했음을 표시한다
        else set(i, j, buf, true);
    }

    //만일 주어진 스도쿠 판에 맞게 모든 값을 입력했다면
    //스도쿠 판을 양식에 맞게 출력한다
    if (flag && fill(0)) for (int i = 0; i < 9; i++)
    {
